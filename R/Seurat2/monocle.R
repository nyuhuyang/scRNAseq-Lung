...
object_Mono <- importCDS(epithelial, import_all = TRUE)
...
all(row.names(pData(object_Mono)) == names(epithelial@ident))
pData(object_Mono)$manual <- epithelial@ident
table(pData(object_Mono)$manual)
pie <- ggplot(pData(object_Mono), aes(x = factor(1), fill = factor(manual))) +
  geom_bar(width = 1)
...
g1 <- plot_cell_trajectory(object_Mono, color_by = "manual",cell_size = 3)
