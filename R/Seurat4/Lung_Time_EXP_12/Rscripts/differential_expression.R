...
path <- paste0("output/",gsub("-","",Sys.Date()),"/EXP12/")
...
object@meta.data = readRDS( file = "output/Lung_time8_metadata_20220523.rds")

step = c("resolutions","Analysis 1a","Analysis 1b","Analysis 2")[1]

if(step == "resolutions"){
    opts = data.frame(ident = c(rep("SCT_snn_res.0.5",22),
                                rep("SCT_snn_res.1",27),
                                rep("SCT_snn_res.2",42),
                                rep("SCT_snn_res.5",74)),
                      num = c(0:21,
                              0:26,
                              0:41,
                              0:73)
