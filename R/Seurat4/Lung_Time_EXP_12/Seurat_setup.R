...
path <- paste0("output/",gsub("-","",Sys.Date()),"/EXP_12/")
...
#======1.2 load  Seurat =========================
object = readRDS(file = "data/Lung_time8_20220523.rds")
...
for(i in 1:length(df_samples$sample)){
    ...
    print(table(cells))
    meta.data[cells,"day"] = df_samples$regions[i]
...
resolutions = c( 0.01, 1:9/10, 1, 2, 3,4,5)
...
object$day %<>% factor(levels = paste0("D",c(0,3,7,14,21,28,56,122)))

saveRDS(object@meta.data, file = "output/Lung_time8_metadata_20220523.rds")
saveRDS(object, file = "data/Lung_time8_20220523.rds")
...
saveRDS(object, file = "data/Lung_SCT_time8_20220523.rds")
