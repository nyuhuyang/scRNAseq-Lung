...
save(sweep.res_list,file = "output/Lung_30_20210916_sweep.res_list.Rda")
load("output/Lung_30_20210916_sweep.res_list.Rda")
...
for(i in 1:length(object_list)){
    ...
    object_list[[i]] <- doubletFinder_v3(object_list[[i]], PCs = 1:50, 
                                         ...
                                         nExp = nExp_poi, reuse.pANN = "Doublets", sct = T)
    object_list[[i]] <- doubletFinder_v3(object_list[[i]], PCs = 1:50, 
                                         pN = 0.25, pK = maximal_pk[i],
                                         nExp = nExp_poi.adj, 
                                         reuse.pANN = "Doublets",
...
object@meta.data = meta.data
saveRDS(object, file = paste0("data/Lung_SCT_30_20210831.rds"))
