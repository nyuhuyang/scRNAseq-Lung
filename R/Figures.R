...
# TSEN plot====
TSNEPlot.1(object,label.repel = F,do.label = T,do.print = T,label.size = 5,
...
epithelial_ident <- c("9 Basal cells",
                      "8 Mucus-producing cells Epithelial cells",
...
jpeg(paste0(path,"Dotplot_all.jpeg"), units="in", width=10, height=7,res=600)
...
for(i in 2:length(clusters)){
        df <- gde.pair[gde.pair$cluster1.vs.cluster2 %in% cluster,]
        g <- ggplot(df,aes(avg_logFC,-log10(p_val_adj))) + 
                geom_point() + 
                ggtitle(titles[i]) + 
                geom_text_repel(aes(label = gene), 
                                data=df[(df$avg_logFC > 1 | df$avg_logFC < -1)& 
                                         df$pct.1>0.2 & df$p_val_adj < 1.00e-050 ,]) +
