...
object %<>% RenameIdents('S-d' = 'A', '2' = 'C')

groups = c("Epithelial","Structural","Immune")
for(i in 1:length(groups)){
    df <- readxl::read_excel("doc/30 DEGs for dot plots.xlsx",
                             sheet = groups[i])
    ...
    g <- DotPlot.1(sub_object, features = rev(genes),
        ...
        coord_flip() + RotatedAxis()
    jpeg(paste0(path,"Dotplot_",groups[i],".jpeg"), units="in", width=9, height=10,res=600)
    print(g)
    dev.off()
