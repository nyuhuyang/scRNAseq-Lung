...
#======== get cell types info from previous data anlaysis ============
(load(file = "data/Lung_24_20191206.Rda"))
meta.data = object@meta.data
remove(object);GC()
(load(file = "data/Lung_16_distal_20191022.Rda"))
remove(object);GC()
meta.data_COPD <- object@meta.data
colnames(meta.data_COPD) = sub("labels","cell.types",colnames(meta.data_COPD))
meta.data_COPD$barcode = rownames(meta.data_COPD)
meta.data_COPD = meta.data_COPD[!(rownames(meta.data_COPD) %in% rownames(meta.data)),]

df_meta.data = rbind(meta.data[,c("cell.types","Doublets","barcode")],
                     meta.data_COPD[,c("cell.types","Doublets","barcode")])
(load(file = "data/Lung_28_20200103.Rda"))
meta.data_all <- object@meta.data
meta.data_all$barcode = rownames(meta.data_all)
meta.data_all = left_join(meta.data_all,df_meta.data,by = "barcode")
rownames(meta.data_all) = meta.data_all$barcode
table(meta.data_all$cell.types)
(remove <- which(colnames(meta.data_all) %in% "ident"))
meta.data_all = meta.data_all[,-remove]
anyNA(meta.data_all$cell.types)
