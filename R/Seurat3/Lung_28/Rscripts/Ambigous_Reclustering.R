...
if(step == 3){  #16GB
        save.path <- paste0(path,"21-",g_name,"/serial_resolutions/",reduction,"/")
        if(!dir.exists(save.path))dir.create(save.path, recursive = T)
        object = readRDS(file = paste0("data/Lung_28_21-",g_name,"-",reduction,"_20200206.rds"))
        resolutions = c(seq(0.001,0.009, by = 0.001),seq(0.01,0.09, by = 0.01),seq(0.1,5, by = 0.1))
        for(i in 49:length(resolutions)){
...
if(step == 4){  #16GB
        save.path <- paste0(path,"21-",g_name,"/DEGs/",reduction,"/")
        if(!dir.exists(save.path))dir.create(save.path, recursive = T)
        object = readRDS(file = paste0("data/Lung_28_21-",g_name,"-",reduction,"_20200206.rds"))
        resolutions = 5
        i=1
        ...
        system.time(Lung_markers <- FindAllMarkers.UMI(object, 
                                                       ...
                                                       test.use = "MAST"))
        Lung_marker = Lung_markers[Lung_markers$p_val_adj<0.05,]
        write.csv(Lung_marker,paste0(path,"Lung_28-FC0.1_res=5_pca.csv"))
}